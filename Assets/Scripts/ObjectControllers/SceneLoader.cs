...
public class SceneLoader : MonoBehaviour
{
    ...
    public async void Load()
    {
        ...
        switch (loadType)
        {
            case LoadType.Scene:
                ...
                await Load(SceneManager.LoadSceneAsync(targetSceneIndex, LoadSceneMode.Additive));
            ...
            case LoadType.Chunk:
                GameObject.FindWithTag("Player").transform.position = targetTransform.position;
                GameObject.FindWithTag("CameraHold").transform.position = targetTransform.position + new Vector3(0, 0, -10); ;
                break;
        }
        ...
        while (sceneControl == null)
        {
            try
            {
                ...
                foreach (GameObject gameObject in gameObjects)
                {
                    ...
                    if (sc != null && sc.sceneNum == targetSceneIndex)
                    {
                        sceneControl = sc;
                        break;
                    }
    ...
    public void SetSceneLoaderContent(int sceneIndex)
    {
        loadType = LoadType.Scene;
        targetSceneIndex = sceneIndex;
    }

    public void SetSceneLoaderContent(Transform transform)
    {
        loadType = LoadType.Chunk;
